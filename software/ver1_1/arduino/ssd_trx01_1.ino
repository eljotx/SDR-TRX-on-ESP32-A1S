...
//Changpuak for amidon core calculator https://www.changpuak.ch/electronics/amidon_toroid_calculator.php
...
#include ".//graph//en_r.h" //encoder right/left icons in setup
#include ".//graph//en_l.h"
...
#define Brg_addr 0x68  //Bridge addr addr MCP3424
...
bool rxtx_checked = true;
bool swr_pwr = false;  //display swr during transmission
bool forw_state = false;
bool back_state = false;
bool forw_started = false;
bool back_started = false;
bool rit = false;

unsigned long long F1 = 350000000ULL;  //basic freq
unsigned long long F2 = 700000000ULL;  //spare freq
...
unsigned long long Frit;
unsigned long long Ftmp;
...
unsigned long delay3 = 5;    //used for button/function establish
unsigned long delay4 = 350;  // tx/rx blocking
unsigned long delay5 = 333;
signed long TxRx_delay;  // = 500;  //delay when change from Tx to Rx in cw
unsigned long oldMs1 = 0;
unsigned long oldMs2 = 0;
unsigned long oldMs3 = 0;
unsigned long oldMs4 = 0;
unsigned long oldMs5 = 0;
unsigned long curMs1;
unsigned long curMs2;
unsigned long curMs3;
unsigned long curMs4;
unsigned long curMs5;
...
int El_key_speed = 9;                                                                                                //El_key_speed initial 14 (9+6) groups/min
int16_t x_of = 2;
int16_t y_of = 2;
int m_digit = 0;
...
int Arw_read_old = 0;
int buttonFirst;
byte Arw_table[52];  //initial arw levels with zero
byte Arw_index = 52;
byte data_s;
...
#define Arw_color04 0x2FE0
#define Arw_color05 0x57E0
#define Arw_color06 0x7FE0
#define Arw_color07 0xA760
#define Arw_color08 0xCF60
#define Arw_color09 0xFF70
#define Arw_color10 0xFEA0
#define Arw_color11 0xFD80
#define Arw_color12 0xFC60
#define Arw_color13 0xFB40
#define Arw_color14 0xFA00
#define Arw_color15 0xF800

float forw; //PWR/SWR variable def
float back;
float swr;
float pwr;
int hiByte;
...
void setup(void) {
  ...
  // Use serial port
  Serial.begin(115200);
  analogSetAttenuation(ADC_11db);  // Sets range up to approx 3.3V
  //AudioLogger::instance().begin(Serial, AudioLogger::Error);

  //create a task that executes the Task0code() function, with priority 1 and executed on core 0
  //xTaskCreatePinnedToCore(Task0code, "Task0", 25000, NULL, 1, NULL, 0);
  xTaskCreatePinnedToCore(Task0code, "Task0", 25000, NULL, 1, &Task0, 0);
  ...
  //set A/D setup for PCA3424
  //wire1.beginTransmission(Brg_addr);  //Initial setting for RF Attn Brg_addr -> 104
  //wire1.write(128);                // config Ch1 continous, gain = 1
  //wire1.endTransmission();
  //sleep(5);
  //wire1.beginTransmission(Brg_addr);  //Initial setting for RF Attn Brg_addr -> 104
  //wire1.write(160);                // config Ch2 continous, gain = 1
  //wire1.endTransmission();
  ...
  fftGraph();  // lower area dedicated for future fft visualisation
  ...
  tft.drawString("Rof", But_x, But_y + 5 * But_sp_y + 10);
...
void loop(void) {
  ...
  if (buttonPressed() > 3) {  //detection Rx or Tx -> Rx mode

    if (rxtx_checked == false) {
      if (millis() - oldMs4 > delay4) {  //delay for blocking tx/rx state
        wire1.beginTransmission(Att_addr);
  ...
  //    Serial.println(amplitude);
    ...
    if (CwSsb_state == true) {  //Cw mode
      ...
      tft.fillCircle(RxTx_led_x, RxTx_led_y, 8, TFT_RED);    //red lamp when TX
      if (UpDn_state == true) {
        if (rit == false) {
          si5351->set_freq_manual(F1 + 80000ULL, pll_tmp, SI5351_CLK2);
        } else {
          si5351->set_freq_manual(Frit + 80000ULL, pll_tmp, SI5351_CLK2);
        }
      } else {
        if (rit == false) {
          si5351->set_freq_manual(F1 - 80000ULL, pll_tmp, SI5351_CLK2);
      ...
      if (rit == false) {
        ...
        tft.drawString("Ron", But_x, But_y + 5 * But_sp_y + 10);
      }

      tft.setTextColor(But_text);
      if (swr_pwr == false) {
        tft.drawString("Pwr", But_x, But_y + 5 * But_sp_y + 10);
      } else {
        tft.drawString("Swr", But_x, But_y + 5 * But_sp_y + 10);
      }

      oldMs1 = millis();
      while ((millis() - oldMs1) < TxRx_delay) {  //checking if still Tx mode
        static int pos = 0;
        if ((forw_started == false) && (forw_state == false)) {
          wire1.beginTransmission(Brg_addr);
          wire1.write(128);
          wire1.endTransmission();
          forw_started = true;
          oldMs3 = millis();
        }
        if ((forw_started == true) && (((millis() - oldMs3) >= 5))) {
          wire1.requestFrom(Brg_addr, 3);
          hiByte = wire1.read();
          loByte = wire1.read();
          cfByte = wire1.read();
          wire1.endTransmission();
          forw = (hiByte << 8) + loByte;
          forw_state = true;
          forw_started = false;
          wire1.beginTransmission(Brg_addr);
          wire1.write(160);
        ...
        if ((back_started == true) && (((millis() - oldMs3) >= 5))) {
          wire1.requestFrom(Brg_addr, 3);
          hiByte = wire1.read();
          loByte = wire1.read();
          cfByte = wire1.read();
          wire1.endTransmission();
          back = (hiByte << 8) + loByte;
          back_state = true;
        }
        ...
        if ((forw_state == true) && (back_state == true)) { //checking PWR/SWR A/D convertion
          if (millis() - oldMs5 > delay5) { //checking convertion time
            if (back > forw) {
              back = forw;
            }
            if (back < forw) {
              float gamma = back / forw;
              //swr = (1 + gamma)/(1 - gamma);
              swr = (forw + back) / (forw - back);
              //swr = forw/back;
            } else {
              swr = 20.0;
            }
            pwr = int(0.03262 + forw * (2.761e-3 + forw * 14.6e-6));
            if (swr_pwr == false) {
              Arw_level = map(pwr, 0, 20, 0, 15);
            } else {
              if (swr < 1.2) {
                Arw_level = 0;
              } else if (swr > 1.2 && swr <= 1.5) {
                Arw_level = 1;
              } else if (swr > 1.5 && swr <= 1.75) {
                Arw_level = 2;
              } else if (swr > 1.75 && swr <= 2.0) {
                Arw_level = 3;
              } else if (swr > 2.0 && swr <= 2.25) {
                Arw_level = 4;
              } else if (swr > 2.25 && swr <= 2.5) {
                Arw_level = 5;
              } else if (swr > 2.5 && swr <= 2.8) {
                Arw_level = 6;
              } else if (swr > 2.8 && swr <= 3.1) {
                Arw_level = 7;
              } else if (swr > 3.1 && swr <= 3.6) {
                Arw_level = 8;
              } else if (swr > 3.6 && swr <= 4.0) {
                Arw_level = 9;
              } else if (swr > 4.0 && swr <= 5.0) {
                Arw_level = 10;
              } else if (swr > 5.0 && swr <= 7.0) {
                Arw_level = 11;
              } else if (swr > 7.0 && swr <= 9.0) {
                Arw_level = 12;
              } else if (swr > 9.0 && swr <= 11.0) {
                Arw_level = 13;
              } else if (swr > 11.0 && swr <= 15.0) {
                Arw_level = 14;
              } else if (swr > 15.0) {
                Arw_level = 15;
              }
            ...
            //char buf[40];
            //sprintf(buf,"Power = %d , SWR = %s", pwr,swr);
            //Serial.println(swr);
            Meter(Arw_level_old, Arw_level);
            Arw_level_old = Arw_level;
            forw_state = false;
        ...
        buttonFirst = buttonPressed();  //readout button state -> KEY. PTT, DASH, DOT
        if (buttonFirst == 1) {
          oldMs1 = millis();             //zeroing cw count due to pressed key
          digitalWrite(Cwk_port, HIGH);  //
          analogWrite(Cwt_port, 127);
          si5351->output_enable(SI5351_CLK2, 1);
        } else if (buttonFirst == 2) {
          oldMs1 = millis();
          oldMs2 = oldMs1;
          ...
          while (millis() - oldMs2 < El_key_time[El_key_speed]) {  //generating dot space
            ...
            encoder->tick();  // just call tick() to check the state, sppeed change during keing 
            int newPos = encoder->getPosition();
            if (pos != newPos) {
              tft.setTextDatum(BC_DATUM);
              tft.setFreeFont(&FreeSans9pt7b);
      ...
      if (Filter_index != 1) {            //if filter is different than 2400
        ...
        Filter_index = 1;
        isFilterChanged = 1;  //forcing setting Tx filter as 24
      }
      buttonFirst = buttonPressed();
      while (buttonFirst < 3 and buttonFirst >= 0) {  //Tx mode when PTT pushed
        buttonFirst = buttonPressed();
      }
...
int buttonPressed() {
  int button_nr;
  int button_value = analogRead(But2s);  //readout button state
  if (button_value < 3622) {             //checking if button pressed
    if (button_value < 3622 && button_value >= 2825) {
      button_nr = 6;  //button F1
    } else if (button_value < 2825 && button_value >= 2160) {
      button_nr = 5;  //button F2
    } else if (button_value < 2160 && button_value >= 1605) {
      button_nr = 4;  //button F3
    } else if (button_value < 1605 && button_value >= 1035) {
      button_nr = 3;  //button dash
    } else if (button_value < 1035 && button_value >= 460) {
      button_nr = 2;  //button dot
    } else if (button_value < 460) {
      button_nr = 1;  //button key/ptt
    }
  } else {
    button_nr = 7;  // no button pressed
  }
  return button_nr;  //return number pressed button
}


void fftGraph() {  //under development
  int tmp1;
  int tmp2;
  Arw_index++;  // change table index
  if (Arw_index > 51) {
    Arw_index = 0;
    tmp2 = Arw_index;
  }
  Arw_table[Arw_index] = Arw_level;
  for (tmp1 = 0; tmp1 < 52; tmp1++) {  //drawing current fft area
    byte y = 236 - tmp1;               //start from most low line
    tmp2 = Arw_index - tmp1;
    if (tmp2 < 0) {
      tmp2 = 51 + tmp2;
    }
    if (Arw_table[tmp2] == 0) {
      tft.drawFastHLine(2, y, 241, Arw_color00);
    } else  //readout value for current line from bottom to top
      if (Arw_table[tmp2] == 1) {
        tft.drawFastHLine(2, y, 241, Arw_color01);
      } else if (Arw_table[tmp2] == 2) {
        tft.drawFastHLine(2, y, 241, Arw_color02);
      } else if (Arw_table[tmp2] == 3) {
        tft.drawFastHLine(2, y, 241, Arw_color03);
      } else if (Arw_table[tmp2] == 4) {
        tft.drawFastHLine(2, y, 241, Arw_color04);
      } else if (Arw_table[tmp2] == 5) {
        tft.drawFastHLine(2, y, 241, Arw_color05);
      } else if (Arw_table[tmp2] == 6) {
        tft.drawFastHLine(2, y, 241, Arw_color06);
      } else if (Arw_table[tmp2] == 7) {
        tft.drawFastHLine(2, y, 241, Arw_color07);
      } else if (Arw_table[tmp2] == 8) {
        tft.drawFastHLine(2, y, 241, Arw_color08);
      } else if (Arw_table[tmp2] == 9) {
        tft.drawFastHLine(2, y, 241, Arw_color09);
      } else if (Arw_table[tmp2] == 10) {
        tft.drawFastHLine(2, y, 241, Arw_color10);
      } else if (Arw_table[tmp2] == 11) {
        tft.drawFastHLine(2, y, 241, Arw_color11);
      } else if (Arw_table[tmp2] == 12) {
        tft.drawFastHLine(2, y, 241, Arw_color12);
      } else if (Arw_table[tmp2] == 13) {
        tft.drawFastHLine(2, y, 241, Arw_color13);
      } else if (Arw_table[tmp2] == 14) {
        tft.drawFastHLine(2, y, 241, Arw_color14);
      } else if (Arw_table[tmp2] == 15) {
        tft.drawFastHLine(2, y, 241, Arw_color15);
      }
  }
}
...
void Settings() {
  ...
  if (rot_enc == false){
    tft.pushImage(13, 138, 60,61, en_r); //drawing encoder icon depending of state
  }
  else {
    tft.pushImage(13, 138, 60,61, en_l);
  }
  tft.setFreeFont(SmallFont);
  ...
  while (saved == false) {
    bool pressed = tft.getTouch(&t_x, &t_y);
    if (pressed) {
      if (t_x < 73 && t_y > 138 && t_y < 198) { 
        if (rot_enc == true){
          rot_enc = false;
          tft.pushImage(13, 138, 60,61, en_r);
        }
        else{
          rot_enc = true;
          tft.pushImage(13, 138, 60,61, en_l);
